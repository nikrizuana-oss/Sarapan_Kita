...
(function(){
  ...
  /* ---------------- RENDER: PLATFORM ORDER (GoFood, ShopeeFood, dll) ---------------- */
  function renderPlatformOrder(){
    const section = $("#platformSection");
    const po = CONFIG.platformOrder;
    if(!po || !po.aktif || !po.daftar){ if(section) section.hidden = true; return; }

    const aktifList = po.daftar.filter(p => p.aktif !== false && p.url);
    if(aktifList.length === 0){ section.hidden = true; return; }

    $("#platformTitle").textContent = po.judul || "Bisa Juga Pesan Lewat";
    $("#platformList").innerHTML = aktifList.map(p => `
      <a href="${p.url}" target="_blank" rel="noopener" class="platform-btn" style="--platform-color:${p.warna || "#1B1815"}">
        ${p.image ? `<img src="${p.image}" alt="${escapeHtml(p.nama)}">` : `<span class="platform-dot"></span>`}
  ...
  function renderPromo(){
    ...
    wrap.innerHTML = CONFIG.promo.map(p => `
      <div class="promo-card ${p.image ? "has-image" : ""}">
        ${p.image ? `<div class="promo-photo"><img src="${p.image}" alt="${escapeHtml(p.title)}"></div>` : ""}
  ...
  function renderMenu(){
    ...
    wrap.innerHTML = items.map(item => {
      const habis = item.stok !== undefined && item.stok !== null && item.stok <= 0;
      const stokLabel = (item.stok !== undefined && item.stok !== null)
        ? (habis ? `<span class="stok-badge habis">Habis</span>` : `<span class="stok-badge">Sisa ${item.stok}</span>`)
        : "";
      return `
      <div class="menu-card ${habis ? "habis" : ""}" data-id="${item.id}">
        <div class="menu-photo">
          ${item.image ? `<img src="${item.image}" alt="${escapeHtml(item.nama)}">` : (item.emoji || "🍽️")}
          ${habis ? `<span class="habis-overlay">Habis</span>` : ""}
        </div>
        <div class="menu-info">
          ...
          <div class="menu-bottom">
            ...
            ${stokLabel}
            <button class="add-btn" data-open="${item.id}" aria-label="Tambah ${escapeHtml(item.nama)}" ${habis ? "disabled" : ""}>+</button>
          </div>
        </div>
      </div>
    `;
    ...
    $$("[data-open]", wrap).forEach(btn => {
      btn.addEventListener("click", () => { if(!btn.disabled) openItemModal(btn.dataset.open); });
    });
